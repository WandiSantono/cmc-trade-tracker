...
# Missing balances and holding-cost details remain missing.
if col_balance:
...
if col_balance:
    ...
    # Opening balance: the last known balance before the selected start date.
    ...
    # Ending balance: the last known balance on or before the selected end date.
...
# Closed-trade P&L is account-wide for the selected date range.
...
# Keep CMC's cashflow sign:
# negative = expense; positive = credit.
commission_net_cost = float(
    commission_rows[col_amount].sum()
)


# Holding costs: use the Account row where available. Do not also add
# position-level rows for the same timestamp.
...
def detail_holding_cashflow(row):
    """Get a position-level holding cashflow while preserving its sign."""
    ...
    # If the available detailed values are zero, preserve the zero value.
    for candidate in candidates:
        if candidate is None:
            continue
...
# Main account figures are displayed across the first row.
...
# Supporting figures are displayed on a separate row.
...
def build_cost_part(source_rows, cost_type, cashflow_values):
    """Build cost-table rows, keeping the CMC cashflow sign."""
    part = source_rows[

...
st.caption("Smart % Profit • Oldest valid Balance + Manual override")
st.markdown("---")
...
# Current Balance = last valid balance in the whole file
if col_balance is not None:
    valid_current = df[col_balance][df[col_balance] != 0]
    current_balance = valid_current.iloc[-1] if len(valid_current) > 0 else df[col_amount].sum()
else:
    current_balance = df[col_amount].sum()

# Oldest valid Balance in the selected period (this is the key fix)
oldest_balance = current_balance  # fallback
if col_balance is not None and len(df_period) > 0:
    ...
    if len(valid_oldest) > 0:
        oldest_balance = valid_oldest.iloc[0]   # first non-zero balance = oldest

# Manual override
st.sidebar.markdown("---")
st.sidebar.subheader("💰 Starting Capital for % Profit")

use_manual = st.sidebar.checkbox("Manually enter Starting Capital", value=False)

if use_manual:
    starting_capital = st.sidebar.number_input(
        "Enter Starting Capital (SGD)",
        min_value=0.0,
        value=float(round(oldest_balance, 2)),
        step=50.0,
        format="%.2f"
    )
else:
    starting_capital = oldest_balance
    st.sidebar.success(f"Using oldest valid balance:\n**SGD {oldest_balance:,.2f}**")
...
if starting_capital != 0:
    pct_profit = ((current_balance - starting_capital) / abs(starting_capital)) * 100
else:
    pct_profit = 0.0
...
st.caption(f"Starting Capital used: **SGD {starting_capital:,.2f}** "
           f"{'(manual override)' if use_manual else '(oldest valid balance in selected period)'}")
st.markdown("---")
...
st.success("✅ Oldest Balance now correctly finds the first real number (e.g. 1,999.93)!")
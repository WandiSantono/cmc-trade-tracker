...
st.caption("Smart Starting Capital + Correct Commission & Holding Costs")
st.markdown("---")
...
# ====================== CURRENT BALANCE + SMART STARTING CAPITAL ======================
...
smart_capital = None
if col_balance is not None:
    deposit_rows = df[deposit_mask_all]
    if len(deposit_rows) > 0:
        smart_capital = deposit_rows.iloc[-1][col_balance]
...
if method == "Smart (Recommended)":
    starting_capital = smart_capital if (smart_capital is not None and smart_capital > 0) else oldest_balance
    method_note = "Smart (after last deposit)" if smart_capital else "Smart fallback → Oldest"
elif method == "Oldest valid Balance":
    starting_capital = oldest_balance
    method_note = "Oldest valid balance"
else:
    starting_capital = st.sidebar.number_input(
        "Enter Starting Capital (SGD)",
        min_value=0.0,
        value=float(round(smart_capital or oldest_balance or 2000, 2)),
        step=50.0,
        format="%.2f"
    )
    method_note = "Manual"
...
mask_trades = (trades[col_date].dt.date >= date_range[0]) & (trades[col_date].dt.date <= date_range[1])
...
if total_deposits != 0:
    st.caption(f"Total Deposits detected: **SGD {total_deposits:,.2f}**")
st.markdown("---")
...
# Holding Cost - Correct logic
is_holding = type_period.str.contains("holding cost", na=False)
holding_rows = df_period[is_holding].copy()

def get_holding_value(row):
    if col_holding and pd.notna(row.get(col_holding)) and row.get(col_holding) != 0:
        return row[col_holding]
    if col_holding_total and pd.notna(row.get(col_holding_total)) and row.get(col_holding_total) != 0:
        return row[col_holding_total]
    return row[col_amount]

if len(holding_rows) > 0:
    holding_rows["Cost"] = holding_rows.apply(get_holding_value, axis=1)
    holding_total = holding_rows["Cost"].sum()
else:
    holding_total = 0.0
...
# ===== COST ROWS TABLE (Sorted by Date/Time as default) =====
with st.expander("🔍 See actual cost rows (sorted by Date/Time)", expanded=False):
    # Commission rows
    comm_rows = df_period[is_commission][[col_date, col_type, col_product, col_amount]].copy()
    comm_rows = comm_rows.rename(columns={col_amount: "Cost"})
    comm_rows["Cost Type"] = "Commission"
    
    # Holding rows
    if len(holding_rows) > 0:
        hold_display = holding_rows[[col_date, col_type, col_product, "Cost"]].copy()
        hold_display["Cost Type"] = "Holding"
        all_costs = pd.concat([comm_rows, hold_display], ignore_index=True)
    else:
        all_costs = comm_rows
    
    # Sort by Date/Time (newest first) - Default setting
    all_costs = all_costs.sort_values(col_date, ascending=False).reset_index(drop=True)
    
    # Reorder columns nicely
    all_costs = all_costs[[col_date, "Cost Type", col_type, col_product, "Cost"]]
    
    st.dataframe(all_costs, use_container_width=True, height=400)
    
    st.caption("Table is sorted by Date/Time (newest on top). You can click the column header to re-sort if needed.")
    st.write(f"**Sum of Cost column = Total Costs: SGD {total_costs:,.2f}**")
...
st.success("✅ Cost table is now sorted by Date/Time (newest first) by default!")